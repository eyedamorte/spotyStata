...
const ArtistRate: FC<ArtistRatePropsType> = ({ releases }) => {
  ...
  useEffect(() => {
    if (releases) {
      ...
      setInstrumentalness(
        getAverage(features.map((person) => person.instrumentalness)) || 0
      );
      setEnergy(getAverage(features.map((person) => person.energy)) || 0);
      setDanceability(
        getAverage(features.map((person) => person.danceability)) || 0
      );
    ...
    <div style={{ margin: 20, padding: 20, backgroundColor: "white" }}>
      ...
      <Form layout={"vertical"}>
        ...
        <Form.Item
          tooltip={{
            title:
              "A confidence measure from 0 to 10 of whether the track is acoustic. 10 represents high confidence the track is acoustic.",
            icon: <InfoCircleOutlined />,
          }}
          style={{ marginBottom: 0 }}
